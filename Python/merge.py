...
if args['c']!="%#$":
    def operation(operatedOnList,operation,delim):
        if operation=="sum":
...
def merge(contacts,cols,commands,d):
    i=0
...
def processCommands(contacts,cols,commands,dashDelim):
    if len(cols)>0:
...
def createHeader(originalHeader,cols,commands,oldHeaderIsUsable):
...
reMerge=merge(sorted(res),[x-1 for x in cols],commands,args['d'])
...
res=processCommands(res,[x-1 for x in cols],commands,args['delim'])
