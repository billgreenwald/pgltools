...
from ..Module.PyGLtools.column_flip import formatbedpe
...
def test_column_flip(capfd):
    ...
    expected_results_file = str(
        ASSET_DIR / "fixtures/column_flip/column_flip_expected_results.pgl"
    )
    ...
    results_not_header = sorted(
        results[1:],
        key=lambda x: (x[0], int(x[1]), int(x[2]), x[3], int(x[4]), int(x[5])),
    )
