...
from ..Module.PyGLtools.samTopgl import samTopgl, formatContacts
...
def test_samTopgl():
    ...
    expected_results_file = str(
        ASSET_DIR / "fixtures/samToPgl/samToPgl_expected_results.pgl"
    )

    header, res = samTopgl(input_file, "\t", 1000)
    results = [line.split() for line in formatContacts(res)]
    results = sorted(
        results, key=lambda x: (x[0], int(x[1]), int(x[2]), x[3], int(x[4]), int(x[5]))
    )
